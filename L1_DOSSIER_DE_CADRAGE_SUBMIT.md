...
|---|---|---|
| GetTransfer | Marketplace internationale de transferts privés (aéroport, ville) | Self-service 24h/24, paiement en ligne instantané, notation chauffeur |
| Mozio | Agrégateur de transport terrestre B2B/B2C, API pour voyagistes | Connexion directe aux systèmes hôtels/agences, catalogue mondial |
| Autocars.com | Portail de mise en relation groupes + autocars en France | Comparaison multi-transporteurs, espace client autonome |
| Agences locales (Havas, PME) | Vente packagée (transport + hébergement + activités) | Offre clé en main, fidélisation, relations entreprises établies |
...
Cinq tables dans Airtable, toutes modifiables sans code par NeoTravel.
...
Les étapes 4 (options) et 5 (marge) de `calculer_devis()` lisent directement `Matrice_Options` et `Parametres_Globaux`. Changer la marge ou le prix d'une option ne nécessite aucune modification de code.
...
J1 ─────────── J3 ─────────── J6 ─────────── J8
   ...
   Structurer        Automatiser      Intégrer IA + démo
```

| Phase | Livrables clés | Responsable |
|---|---|---|
| **1 — Structurer (J1-J3)** | Airtable (tables + matrices + dashboard) · Next.js init · Vercel + Resend configurés | Gendell + Inde |
| **2 — Automatiser (J3-J6)** | Formulaire 13 questions · IF node passagers · `calculer_devis()` · PDF · Relances WF2 + idempotence · Tests bout en bout | Gendell + Inde |
| **3 — Intégrer IA (J6-J8)** | Gemini WF1 · Prompt système · Page `/clarification` · Stabilité démo · L3 + Slides | Gendell + Inde |

---

## 12. Maquettes et Flux Utilisateur

### Interface principale — formulaire Next.js

Une question par écran, barre de progression visible. Pas de chatbot, pas de formulaire long.

| Étape | Question | Type d'input |
|:---:|---|---|
| 1 | Type de client | Boutons (Particulier / Entreprise / Association / Collectivité) |
| 2-4 | Nom · Email · Téléphone | Texte / Email / Téléphone |
| 5-6 | Ville de départ · Ville de destination | Texte (autocomplétion) |
| 7-8 | Date de départ · Nombre de passagers | Date picker · Numérique |
| 9-11 | Type trajet · Urgence · Distance (km) | Boutons · Boutons · Numérique |
| 12-13 | Options · Commentaires | Checkboxes · Texte long (optionnel) |

### Page `/clarification?id={lead_id}`

Le client arrive sur une page dédiée avec un widget de chat qui lui pose une seule question : celle qui manque. Quand le champ est renseigné, la page relance le webhook WF1. Le parcours reprend depuis la qualification IA.

### Flow Gemini dans n8n

| | Détail |
|---|---|
| **Input** | JSON formulaire : tous les champs texte |
| **Tâche** | Évalue la complétude textuelle uniquement. Ne vérifie pas le nombre de passagers (déjà géré par IF node) |
| **Output** | `{ completeness_score: int, missing_fields: string[] }` |
| **Branche** | ≥ 70 % → `calculer_devis()` · < 70 % → `/clarification` |
